...
func OpenConnection() *gorm.DB {
	dialect := mysql.Open("phpmyadmin:blackot@tcp(localhost:3306)/belajar_golang_gorm?charset=utf8mb4&parseTime=True&loc=Local")
...
func TestSqlRow(t *testing.T) {
	var sample Sample
	row := db.Raw("select id, name from sample where id = ?", "1").Row()
	var id, name string
...
func TestTakePreloadJoinOneToMany(t *testing.T) {
	var users []User
	err := db.Model(&User{}).Preload("Addresses").Joins("Wallet").Take(&users, "users.id = ?", "1").Error
	assert.Nil(t, err)
}
...
func TestAggregationGroupByAndHaving(t *testing.T) {
	...
	err := db.Model(&Wallet{}).Select("sum(balance) as total_balance", "min(balance) as min_balance", "max(balance) as max_balance", "avg(balance) as avg_balance").Joins("User").Group("User.id").Having("sum(balance) > ?", 500000).Find(&results).Error
